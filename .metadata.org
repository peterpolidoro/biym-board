...
inkscape --export-area-drawing --export-overwrite <<pcb-dir>>*.svg
#+END_SRC
...
#+BEGIN_SRC python :noweb yes :exports results :results output raw
from pathlib import Path
path_strings = ['<<pcb-dir>><<name>>-F_Silkscreen.svg',
                '<<pcb-dir>><<name>>-B_Silkscreen.svg',
                '<<pcb-dir>><<name>>-User_Drawings.svg']
for path_string in path_strings:
    path = Path(path_string)
    if path.is_file():
        print(f'[[file:./{path}]]\n')
